<?xml version="1.0" encoding="UTF-8"?>


<!-- Licensed to the Apache Software Foundation (ASF) under one or more contributor
	license agreements. See the NOTICE file distributed with this work for additional
	information regarding copyright ownership. The ASF licenses this file to
	you under the Apache License, Version 2.0 (the "License"); you may not use
	this file except in compliance with the License. You may obtain a copy of
	the License at http://www.apache.org/licenses/LICENSE-2.0 Unless required
	by applicable law or agreed to in writing, software distributed under the
	License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS
	OF ANY KIND, either express or implied. See the License for the specific
	language governing permissions and limitations under the License. -->


<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xsi:schemaLocation="http://maven.apache.org/SETTINGS/1.0.0 http://maven.apache.org/xsd/settings-1.0.0.xsd">
	<pluginGroups>
	</pluginGroups>

	<servers>
		<server>
			<id>artifactory.snapshots</id>
			<username>502447016</username>
			<password>AP7dXKehQdPuji6thyhiXaqZZMY</password>
		</server>
		<server>
			<id>artifactory.releases</id>
			<username>502447016</username>
			<password>AP7dXKehQdPuji6thyhiXaqZZMY</password>
		</server>
		<server>
			<id>artifactory.repo</id>
      <username>502447016</username>
			<password>AP7dXKehQdPuji6thyhiXaqZZMY</password>
		</server>
		<server>
			<id>predix.repo</id>
			<username>predix-ext2</username>
			<password>AKCp5Z2rqABWNTUYFPHyRYbJeyeZ7m9Cv7QmtFycmwoU6UxbwTzStqr5S8nw6hHoJ4ZjQQXPS</password>
		</server>
	</servers>
<!--
        <proxies>
                <proxy>
                        <id>optional</id>
                        <active>true</active>
                        <protocol>http</protocol>
                        <host>proxy-src.research.ge.com</host>
                        <port>8080</port>
                        <nonProxyHosts>*.ge.com|api.ges-apps.ice.ge.com|api.grc-apps.svc.ice.ge.com|login.grc-apps.svc.ice.ge.com|loggregator.grc-apps.svc.ice.ge.com|uaa.grc-apps.svc.ice.ge.com|console.grc-apps.svc.ice.ge.com</nonProxyHosts>
                </proxy>
                <proxy>
                        <id>optional-https</id>
                        <active>true</active>
                        <protocol>https</protocol>
                        <host>proxy-src.research.ge.com</host>
                        <port>8080</port>
                        <nonProxyHosts>*.ge.com|api.ges-apps.ice.ge.com|api.grc-apps.svc.ice.ge.com|login.grc-apps.svc.ice.ge.com|loggregator.grc-apps.svc.ice.ge.com|uaa.grc-apps.svc.ice.ge.com|console.grc-apps.svc.ice.ge.com</nonProxyHosts>
                </proxy>
        </proxies>
-->
	<profiles>
		<profile>
			<id>sonar</id>
			<activation>
				<activeByDefault>false</activeByDefault>
			</activation>
			<properties>
				<sonar.jdbc.url>
					jdbc:mysql://3.39.73.57:3323/sonarqube?useUnicode=true&amp;characterEncoding=utf8
				</sonar.jdbc.url>
				<sonar.jdbc.username>sonar</sonar.jdbc.username>
				<sonar.jdbc.password>sonar</sonar.jdbc.password>
				<sonar.host.url>
					http://3.39.89.84:9000
				</sonar.host.url>
			</properties>
		</profile>

		<profile>
			<id>SnapshotRepository</id>
			<activation>
				<activeByDefault>true</activeByDefault>
			</activation>
			<repositories>
				<repository>
					<id>artifactory.snapshots</id>
					<name>artifactory.snapshots</name>
					<url>https://devcloud.swcoe.ge.com/artifactory/PREDIX-SNAPSHOT</url>
					<layout>default</layout>
					<snapshots>
						<enabled>true</enabled>
					</snapshots>
					<releases>
						<enabled>false</enabled>
					</releases>
				</repository>
				<repository>
					<id>artifactory.releases</id>
					<name>artifactory.releases</name>
					<url>https://devcloud.swcoe.ge.com/artifactory/PREDIX</url>
				</repository>
				<repository>
					<id>central</id>
					<name>central</name>
					<url>http://repo1.maven.org/maven2</url>
					<releases>
						<enabled>true</enabled>
					</releases>
					<snapshots>
						<enabled>true</enabled>
					</snapshots>
				</repository>
				<repository>
					<id>predix.repo</id>
					<name>predix.repo</name>
					<url>https://artifactory.predix.io/artifactory/PREDIX-EXT</url>
					<snapshots>
						<enabled>false</enabled>
					</snapshots>
					<releases>
						<enabled>true</enabled>
					</releases>
				</repository>
			<!--	<repository>
					<id>artifactory.repo</id>
					<name>artifactory.repo</name>
					<url>https://devcloud.swcoe.ge.com/artifactory/repo</url>
				</repository>-->
			</repositories>
		</profile>
	</profiles>

	<activeProfiles>
		<!--<activeProfile>repository</activeProfile>-->
	</activeProfiles>

</settings>
