  ...
  .then(function (body){
    ...
    for(var i = 0; i < body.length; i++){
      for (var j = 0; j < body[i].content.length; j++){
        ...
        var humidity = JSON.stringify(body[i].content[j].measures);
        //console.log(body[i].content[j].assetUid);
        //var tempsql = "UPDATE Data SET (Timestamp, Temp) VALUES ('"+timeStamp+"','"+temp+") WHERE ID = '"+id+"'";
        var tempsql = "INSERT INTO Data (ID, Timestamp, Humidity) VALUES ('"+id+"','"+timeStamp+"','"+humidity+"') ON DUPLICATE KEY UPDATE Timestamp = '"+timeStamp+"', Humidity = '"+humidity+"'";
